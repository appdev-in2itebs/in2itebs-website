...
The homepage hero mounts a WebGL sculpture (`components/hero/hero-3d-loader.tsx` gating, `components/hero/hero-3d.tsx` scene) only when the viewport is ≥1024px, the connection does not report `saveData`, WebGL is available, and the page is not hidden — and only after the page goes idle. A viewport under 1024px, `prefers-reduced-motion: reduce`, data saver, or no WebGL means the scene never mounts at all (`unavailable`, with a matching reason); the static `.hero-ambient` gradient backdrop is what those cases, and every other viewport, see instead. Once mounted, it pauses (and the clock freezes so it does not jump on resume) under the page's own pause control, a hidden tab, or scrolling offscreen. The three.js dependency ships in exactly one lazily loaded chunk, verified by scripts/check-build.mjs at build time to stay under a 700,000-byte budget and to never appear in the initial-JavaScript app-build manifest.
