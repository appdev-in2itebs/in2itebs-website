...
The homepage hero mounts a WebGL sculpture (`components/hero/hero-3d-loader.tsx` gating, `components/hero/hero-3d.tsx` scene) only when every one of these holds: the viewport is ≥1024px, motion is allowed (no `prefers-reduced-motion: reduce`), the connection does not report `saveData`, WebGL is available, and the page is visible — and then only after the page goes idle. Failing any one of them means the scene never mounts at all (`unavailable`, with a matching reason); the static `.hero-ambient` gradient backdrop is what those cases, and every other viewport, see instead. The hidden-page gate is the one that can lift on its own: a tab opened in the background re-evaluates the list on `visibilitychange` rather than staying unavailable for good. Once mounted, it pauses (and the clock freezes so it does not jump on resume) under the page's own pause control, a hidden tab, or scrolling offscreen. The three.js dependency ships in exactly one lazily loaded chunk, verified by scripts/check-build.mjs at build time to stay under a 700,000-byte budget and to never appear in the initial-JavaScript app-build manifest.
