...
YouTube ribbon (2026-09-15 17:08): the homepage ends with a scroll-snap carousel of the channel's latest uploads, newest first, each linking to the video. The catalogue `web/content/youtube.ts` and the thumbnails under `web/public/youtube/` are generated, not hand-written: run `node scripts/fetch-youtube.mjs` from `web/` to refresh them from the channel's public Atom feed (15 most recent uploads, no API key), then commit the result. `tests/youtube.test.ts` checks the catalogue is newest-first with a thumbnail on disk for every entry; `tests/browser/youtube.spec.ts` covers the links and the arrows.
