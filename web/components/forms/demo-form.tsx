...
export function DemoForm() {
  ...
  useEffect(() => {
    ...
    fetch("/api/health/", { signal: controller.signal, cache: "no-store" })
      .then((r) => r.json())
      .then((h) => setDeliveryEnabled(Boolean(h.leadDeliveryConfigured)))
  ...
  useEffect(() => {
    ...
    if (result && ["RISE", "GROW", "Hybrid"].includes(result)) {
    ...
    try {
      ...
      setError(
        error instanceof Error && error.name === "TimeoutError"
          ? "Delivery timed out. Your request may have been received; email us to confirm before retrying."
          : error instanceof Error
            ? error.message
            : "Connection failed. Please email us.",
      );
      setBusy(false);
    ...
    <form onSubmit={submit} className="flex flex-col gap-5">
      ...
      {deliveryEnabled === null && (
        <p role="status" className="text-sm text-foreground-muted">
          Checking whether online enquiries are available…
        </p>
      )}
      {deliveryEnabled === false && (
        ...
        >
          ...
          . This form will not report a request as delivered.
        </p>
      )}
      ...
      <label className="grid gap-2">
        ...
        <input name="company" required minLength={2} maxLength={160} autoComplete="organization" className={field} />
      </label>
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="grid gap-2">
          <span className="text-sm font-semibold">Contact region</span>
          <select name="region" value={code} onChange={(e) => choose(e.target.value)} className={field}>
            {regions.map((r) => (
        ...
        <label className="grid gap-2">
          <span className="text-sm font-semibold">Interest area</span>
          <select name="interest" value={interest} onChange={(e) => setInterest(e.target.value)} className={field}>
            {interests.map((i) => (
      ...
      <label className="grid gap-2">
        ...
        <select
          name="offer"
          value={offer}
          onChange={(e) => setOffer(e.target.value as keyof typeof offers)}
          className={field}
        >
          {Object.entries(offers).map(([key, label]) => (
      ...
      {assessment && (
        <p className="text-sm text-foreground-muted">
          Your indicative assessment: {assessment}. This will be included with your request.
        </p>
      )}
      <label className="grid gap-2">
        <span className="text-sm font-semibold">Message (optional)</span>
        <textarea name="message" rows={5} maxLength={5000} className={field + " resize-y"} />
      </label>
      <p className="text-sm text-foreground-muted">
        We use these details to respond to your enquiry. Read our{" "}
        <Link href="/legal/privacy-notice/" className="text-action underline">
          privacy notice
        </Link>
        . Submitting a request does not book an appointment or subscribe you to marketing.
      </p>
      {error && (
        <p
          ref={errorRef}
          tabIndex={-1}
          role="alert"
          className="rounded-control border border-error p-4 text-sm text-error"
        >
          {error}{" "}
          <a className="underline" href="mailto:info@in2itebs.com">
            Email us
          </a>
          .
        </p>
