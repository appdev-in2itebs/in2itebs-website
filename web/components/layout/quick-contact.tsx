...
export function QuickContact(){
  ...
  return <div ref={root} hidden={nearFooter && !open && !focused} className="fixed bottom-5 right-5 z-30" onKeyDown={event=>{if(event.key==='Escape'){setOpen(false);trigger.current?.focus();}}} onFocusCapture={()=>setFocused(true)} onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node)){setOpen(false);setFocused(false);}}}>
