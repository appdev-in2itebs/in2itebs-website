...
export function QuickContact(){
  ...
  return <div ref={root} className="fixed bottom-5 right-5 z-30" onKeyDown={event=>{if(event.key==='Escape'){setOpen(false);trigger.current?.focus();}}} onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node))setOpen(false);}}>
