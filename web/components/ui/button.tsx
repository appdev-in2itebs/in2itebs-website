...
const variants: Record<Variant, string> = {
  ...
  secondary: "border border-border-strong bg-surface text-foreground hover:bg-surface-subtle hover:border-action focus-visible:ring-offset-surface",
  ghost: "text-foreground hover:bg-surface-subtle hover:text-action focus-visible:ring-offset-surface",
  "on-dark": "bg-on-brand text-brand hover:bg-brand-muted hover:-translate-y-px focus-visible:ring-brand-muted focus-visible:ring-offset-brand",
};
...
export function Button({
  ...
  const inner = (
    <>
      <span>{children}</span>
      {withArrow ? (
        <ArrowUpRight size={16} strokeWidth={1.6} className="transition-transform duration-200 ease-out group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
      ) : null}
    </>
  );
