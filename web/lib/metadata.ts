...
export function pageMetadata(path: string, input: Metadata): Metadata {
  ...
  const absolute =
    typeof input.title === "object" && input.title !== null && "absolute" in input.title
      ? String(input.title.absolute)
      : null;
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      url: canonical,
      images: [{ url: "/opengraph-image/", width: 1200, height: 630, alt: "In2IT EBS: enterprise transformation" }],
    },
    twitter: {
