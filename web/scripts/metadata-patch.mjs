...
for (const file of files) {
  ...
  if (file.includes("[slug]")) {
    ...
    next = next.replace(
      /(export (?:default )?async function (?:generateMetadata|\w+Page)\([^\n]+\{\n)/g,
  ...
  function walk(node, inMetadata = false) {
    if (
      ...
      changes.push({ start: node.initializer.getStart(source), end: node.initializer.end });
    const inside = inMetadata || (ts.isFunctionDeclaration(node) && node.name?.getText(source) === "generateMetadata");
    if (inside && ts.isReturnStatement(node) && node.expression && ts.isObjectLiteralExpression(node.expression))
      changes.push({ start: node.expression.getStart(source), end: node.expression.end });
    ts.forEachChild(node, (n) => walk(n, inside));
  }
  walk(source);
  for (const c of changes.sort((a, b) => b.start - a.start))
    next =
      next.slice(0, c.start) +
      "pageMetadata(" +
      expression +
      ", " +
      next.slice(c.start, c.end) +
      ")" +
      next.slice(c.end);
  ...
  if (next !== old) {
    ...
    patch +=
      "*** Update File: " +
      path.resolve(target).replaceAll("\\", "/") +
      "\n@@\n" +
      a
