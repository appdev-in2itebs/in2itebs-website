...
test('record local first-viewport performance',async({browser})=>{
  ...
  for(const width of [375,1440]) for(const path of ['/','/contact/','/sap-enterprise-solutions/rise-vs-grow/']) {
    ...
    await page.addInitScript(()=>{
      ...
      new PerformanceObserver(list=>{for(const entry of list.getEntries() as (PerformanceEntry & {hadRecentInput:boolean;value:number})[]) if(!entry.hadRecentInput) metrics.cls+=entry.value;}).observe({type:'layout-shift',buffered:true});
    });
    await page.goto(`http://127.0.0.1:3107${path}`);await page.evaluate(()=>document.fonts.ready);
    ...
    samples.push({path,width,...metrics});await context.close();
  }
  console.log('LOCAL_UNTHROTTLED_METRICS',JSON.stringify(samples));
  await test.info().attach('local-performance',{body:JSON.stringify(samples,null,2),contentType:'application/json'});
});
